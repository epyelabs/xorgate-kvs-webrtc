...
WebRTC client for AWS Kinesis Video Streams, built on the
...
This project builds **one** executable from `src/` + the vendored SDK:
...
It *sends* frames (encoder → `appsink` → `writeFrame`) and uses `src/Common.c` for the
signaling / peer-connection glue.

> **Removed 2026-07-28:** this package used to build a second target, `xorgate-kvs-recorder` — a
> cloud viewer-recorder that ran on AWS Fargate, joined a device channel as a viewer and remuxed
> the stream to MP4 in S3. It was deleted along with the rest of the cloud recorder
> (`src/recorder.c`, `src/GstRecorder.c`, the `Dockerfile`/`entrypoint.sh` that built its image,
> and its ECR repo + ECS task definition). Recording is now owned entirely by the device agent
> (`../xorgate-device-agent`, `src/recording/**`), which writes and uploads segments locally.
...
CMakeLists.txt        builds the master, links against the built SDK
