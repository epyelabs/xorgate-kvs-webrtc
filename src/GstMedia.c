...
PVOID sendGstreamerAudioVideo(PVOID args)
{
    ...
    pGstPipelineFile = GETENV(KVS_GST_PIPELINE_ENV_VAR);
    if (pGstPipelineFile != NULL) {
        // logError = FALSE so a missing file doesn't spam ERROR logs; we emit our own warning.
        if (STATUS_SUCCEEDED(readFile(pGstPipelineFile, FALSE, NULL, &fileSize)) && fileSize > 0) {
            pPipelineFromFile = (PCHAR) MEMCALLOC(1, (fileSize + 1) * SIZEOF(CHAR));
            CHK_ERR(pPipelineFromFile != NULL, STATUS_NOT_ENOUGH_MEMORY, "[KVS Gstreamer Master] Failed to allocate pipeline buffer");
            if (STATUS_SUCCEEDED(readFile(pGstPipelineFile, FALSE, (PBYTE) pPipelineFromFile, &fileSize))) {
                pPipelineStr = pPipelineFromFile;
                DLOGI("[KVS GStreamer Master] Using pipeline from file: %s", pGstPipelineFile);
            } else {
                DLOGW("[KVS GStreamer Master] Could not read pipeline file %s; using default pipeline", pGstPipelineFile);
                SAFE_MEMFREE(pPipelineFromFile); // pPipelineStr stays DEFAULT_GST_PIPELINE
            }
        } else {
            DLOGW("[KVS GStreamer Master] Pipeline file %s missing or empty; using default pipeline", pGstPipelineFile);
        }
    }

    // pPipelineStr is always non-NULL (starts at DEFAULT_GST_PIPELINE), so the source-type switch below
    // is an unreachable secondary fallback, retained for reference.
    if (pPipelineStr != NULL) {
        senderPipeline = gst_parse_launch(pPipelineStr, &gError);
    } else
...
CleanUp:
    ...
    SAFE_MEMFREE(pPipelineFromFile);
